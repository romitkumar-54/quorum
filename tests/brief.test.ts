...
function ingest(texts: string[]) {
  const log = new TranscriptLog()
  const builder = new BriefBuilder()
  let t = 0
  for (const text of texts) {
    t += 10_000
    builder.ingest(log.append({ speaker: 'candidate', text, tStart: t }))
  }
  return builder
}

describe('claims and flags', () => {
  it('splits one utterance into separate claims', () => {
    const brief = ingest([DEMO_TRANSCRIPT[0].text]).current()
  ...
  it('flags impact asserted without a number', () => {
    const brief = ingest([DEMO_TRANSCRIPT[0].text]).current()
    expect(brief.flags.map((f) => f.kind)).toContain('unchallenged_impact')
  })

  it('does not flag impact that is quantified', () => {
    const brief = ingest(['It cut p95 latency for users from 400ms to 90ms.']).current()
    expect(brief.flags.map((f) => f.kind)).not.toContain('unchallenged_impact')
  })

  it('detects the contradiction across turns, keeping both timestamps', () => {
    const brief = ingest([DEMO_TRANSCRIPT[1].text, DEMO_TRANSCRIPT[2].text]).current()
  ...
  it('carries evidence back to the exact event', () => {
    const builder = ingest([DEMO_TRANSCRIPT[0].text])
...
describe('difficulty adjusts to performance', () => {
  it('rises on a specific answer', () => {
    expect(ingest(['I used a hash map so lookups are O(1).']).current().difficulty).toBe(3)
  })

  it('falls on a turn with nothing concrete in it', () => {
    expect(ingest(['It made things a lot faster.']).current().difficulty).toBe(1)
  })

  it('still rises when one sentence is concrete and another is hand-wavy', () => {
    ...
    expect(ingest([DEMO_TRANSCRIPT[0].text]).current().difficulty).toBe(3)
  })

  it('leaves difficulty alone for a contradiction', () => {
    const before = ingest([DEMO_TRANSCRIPT[1].text]).current().difficulty
    const after = ingest([DEMO_TRANSCRIPT[1].text, 'We rolled it out gradually.']).current().difficulty
...
describe('the split panel', () => {
  ...
  it('reports the disagreement rather than averaging it away', () => {
  ...
  it('writes verdicts that match the evidence', () => {
  ...
  it('links every verdict back to a moment in the transcript', () => {
